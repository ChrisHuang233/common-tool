...
public class CRC16Util {
	...
	/** CRC1021余式表 */
	static final char TABLE1021[] = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129,
	 ...
	 *             参数错误
	 */
	public static int calculate(final byte[] data) {
	...
	public static int crc16(final byte[] data) {
		...
		for (int n = 0; n < data.length; n++) {
			...
			crc ^= (data[n] & 0xFF);// byte to int, trunc sign
			crc ^= ((crc & 0xFF) >> 4);
			crc ^= (crc << 12) & 0xFFFF;
	 ...
	 *             参数错误
	 */
	public static int crc(final byte[] data) {
		if (data == null || data.length < 1) {
			throw new IllegalArgumentException("数据不能为空！");
		}
		...
		int crc = 0xFFFF;
		for (byte b : data) {
			crc = (crc << 8) ^ TABLE1021[((crc >> 8) ^ b) & 0xFF];
		}
		return crc & 0xFFFF;
	}
	 ...
	 *             参数错误
	 */
	public static int crc(byte[] data, int begin, int length) {
		if (data == null || data.length < 1) {
			throw new IllegalArgumentException("数据不能为空！");
		}
		if (begin < 0 || begin >= data.length) {
			throw new IllegalArgumentException("起始位置超出范围！expect:0 <= begin < " + data.length + " reality:" + begin);
		}
		if (length < 1 || begin + length > data.length) {
			throw new IllegalArgumentException(
					"计算长度超出范围！expect:1 <= length <= " + (data.length - begin) + " reality:" + length);
		}

		int crc = 0xFFFF;
		for (int i = begin; i < begin + length; i++) {
			crc = (crc << 8) ^ TABLE1021[((crc >> 8) ^ data[i]) & 0xFF];
		}
		return crc & 0xFFFF;
	}
